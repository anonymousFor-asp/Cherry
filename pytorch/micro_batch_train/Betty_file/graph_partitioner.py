...
class Graph_Partitioner:  # ----------------------*** split the output layer block ***---------------------
	...
	def simple_gen_K_batches_seeds_list(self):
		...
		if self.selection_method == "random" or self.selection_method == "range" or self.selection_method=="metis":
			...
			print('REG start----................................')
			# src node -> one-hop neighbors
			print('src_nids:', len(self.src_nids_list))
			# train nid: dst_node
			print('dst_nids:', self.output_nids.shape)
			...
			# print("g_edges:", g.num_edges)
			# src_out_degrees = g.out_degrees(g.srcnodes('_N'))
			# print("src_out_degrees:", src_out_degrees.shape)
			# g.edata['w'] = src_out_degrees

			# check the node attributes -> have no attrs
			# node_attributes = g.ndata
			# for node_idx, attributes in list(node_attributes.items())[:5]:
			# 	print(f"Node {node_idx}: {attributes}")
			# src_nodes, dst_nodes = g.edges()
			# shared_neighbors = [len(set(g.successors(src).numpy()) & set(g.successors(dst).numpy())) for src, dst in zip(src_nodes, dst_nodes)]
			# g.edata['weight'] = torch.tensor(shared_neighbors, dtype=torch.float32)
			...
			tt2 = time.time()
			print("Prepare for partition time:", tt2 - tt1)
			...
			tp1 = time.time()
			print("Remove and no_diag time:", tp1 - tt2)
			# direct or undirect -> undirect
			# print('Directed Graph:', auxiliary_graph_no_diag.is_block)
			print("--------Partition Graph Info")
			print("nodes: ", auxiliary_graph_no_diag.number_of_nodes())
			print("edges: ", auxiliary_graph_no_diag.number_of_edges())
			...
			tp2 = time.time()
			print("Betty Partitioin time:", tp2 -tp1)
			res=[]
			for pid in partition:
				nids = partition[pid].ndata[dgl.NID].tolist()
				res.append(sorted(nids))
